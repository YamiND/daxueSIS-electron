...
function removeClassStudentList($mysqli)
{
...
?>
	<option></option>
	 <?php
    if ($stmt = $mysqli->prepare("SELECT userID, userEmail, userFirstName, userLastName FROM users WHERE isStudent AND userID IN (SELECT studentID FROM studentClassIDs WHERE classID LIKE ?)"))
    {
